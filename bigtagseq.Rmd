...
EnhancedVolcano(fem_ec_resOrdered_df,
  ...
  xlim = c(-2, 2),
  ylim = c(0, 5),
  title = "Entorhinal Cortex (females)",
  legendPosition = 'none',
  subtitle = "Differential Expresion",
  max.overlaps = 25
)
...
#try using GSEgo instead of enrichr, can't figure out enrichr 
...
male_hip_dot_graph <- dotplot(male_hip_cluster_gse, showCategory=5, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y = element_text(size = 8)) + ggtitle("Male Hippocampus")
male_hip_dot_graph

fem_hip_dot_graph <- dotplot(fem_hip_cluster_gse, showCategory=5, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y = element_text(size = 8)) + ggtitle("Female Hippocampus")
fem_hip_dot_graph

male_ec_dot_graph <- dotplot(male_ec_cluster_gse, showCategory=5, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y = element_text(size = 6)) + ggtitle("Male Entorhinal Cortex")
male_ec_dot_graph

fem_ec_dot_graph <- dotplot(fem_ec_cluster_gse, showCategory=5, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y = element_text(size = 8)) + ggtitle("Female Entorhinal Cortex")
fem_ec_dot_graph
