...
hip_dds_75 <- hip_dds2[rowSums(counts(hip_dds2) >= 10) >= 34,]
...
soft_power <- 24 #enter soft power here 
temp_cor <- cor
cor <- WGCNA::cor
...
CorLevelPlot(heatmap.data,
             x= names(heatmap.data)[5:6],
             y= names(heatmap.data)[2:4],
...
ggplot(data = midnightblue_hip_eigengene, aes(x = ID, y = value, fill = treatment, color = sex)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("ethanol" = "yellow", "control" = "aquamarine")) +
  scale_color_manual(values = c("male" = "blue", "female" = "red")) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
