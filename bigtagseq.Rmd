...
DESeq object for sex differences 
```{r}
#hippocampus 
#construct DESeq object 
hip_dds<- DESeqDataSetFromMatrix(countData = hip_counts_data,
...
#subset hip_dds to include only samples with ethanol treatment 
eth_hip_dds <- hip_dds[hip_dds$treatment == "ethanol"]

#Run DESeq on object.size
eth_hip_dds<- DESeq(eth_hip_dds)
#hip_res <- results(hip_dds, contrast = c(contrast_males <- results(hip_dds, contrast=c("treatment_ethanol_vs_control", "treatmentethanol", "treatmentcontrol", "sexmale"))
#)) ("condition", "level_to_compare", "base_level")
#summary(hip_res)

eth_hip_res <- results(eth_hip_dds, contrast=c("sex", "male", "female"))


#genes ranked by p-value
hip_resOrdered <- eth_hip_res[order(eth_hip_res$padj),]
