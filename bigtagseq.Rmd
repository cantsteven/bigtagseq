...
Hippocampus DESeq object, did not end up using this 
...
Hippocampus DESeq object treatment + sex, did not end up using this 
...
DESeq object with interaction factor, did not end up using this 
...
hip_resOrdered_3 <- hip_resOrdered_3[abs(hip_resOrdered_3$log2FoldChange) > 0.1, ]
...
```{r}
plotCounts(male_hip_dds, gene="ENSRNOG00000037302", intgroup="treatment")
```
...
yellow_module <- module.gene.mapping %>%
  filter(`bwnet$colors` == "yellow") %>% #filter by color for a specific module 
  rownames()

gene_symbols_yellow <- mapIds(org.Rn.eg.db, keys=yellow_module, column="SYMBOL", keytype="ENSEMBL")
write_lines(gene_symbols_yellow, "yellow_symbols.txt")
...
WGCNA for males in hippocampus
...
#detect outlier genes---------------------------------------------------------------
gsg<- goodSamplesGenes(t(male_hip_counts_data))
summary(gsg)
gsg$allOK #result is false- so there are outliers. we need to find these genes and exclude from analysis 
...
#remove genes that are detected as outliers
data_nooutlier <- male_hip_counts_data[gsg$goodGenes == TRUE,]
...
#expression data needs to be normalized 
#ensuring ronames and column names are identical
all(rownames(male_hip_colData) %in% colnames(data_nooutlier))
all(rownames(male_hip_colData) == colnames(data_nooutlier))
...
#remove all genes with counts > 15 in more than 75% of samples (38 * 0.75 = ~29)
hip_dds_75 <- hip_dds2[rowSums(counts(hip_dds2) >= 15) >= 14,]
nrow(hip_dds_75) #9540 genes left 
...
soft_power <- 10 #enter soft power here 
temp_cor <- cor
cor <- WGCNA::cor
...
bwnet <- blockwiseModules(hip_norm.counts, 
                 ...
                 TOMType = "signed", 
                 power = soft_power, 
                 mergeCutHeight = 0.4, #threshold to merge similar modules 
                 numericLabels = FALSE, #going by colors instead of numbers
                 randomSeed = 1234,
                 verbose = 3)
...
#next we want to see which traits related to either treatment or sex 
#so we have to binarize the variables, ethanol will be 1, and female will be 1
hip_colData_mut <- male_hip_colData %>%
  mutate(treatment_state_bin = ifelse(grepl('ethanol', treatment), 1, 0)) %>%
  #mutate(sex_state_bin = ifelse(grepl('female', sex), 1, 0)) %>%
  dplyr::select(3)
...
write_lines(grey_module, "grey_module.txt")

#converting to gene symbols
gene_symbols_gray <- mapIds(org.Rn.eg.db, keys=grey_module, column="SYMBOL", keytype="ENSEMBL")
write_lines(gene_symbols_gray, "grey_module_symbols.txt")

#red module genes- this one seems to be the most astrocyte related--------- 
red_module <- module.gene.mapping %>%
  filter(`bwnet$colors` == "red") %>% #filter by color for a specific module 
  rownames()
  
# write_lines(lightyellow_module, "red_module.txt")
# write.csv(lightyellow_module, "lightyellow_module.csv")

#converting to gene symbols
gene_symbols_red <- mapIds(org.Rn.eg.db, keys=red_module, column="SYMBOL", keytype="ENSEMBL")
write_lines(gene_symbols_red, "red_module_symbols.txt")
...
male_ec_cluster <- clusterdf(fem_hip_resOrdered_df)
